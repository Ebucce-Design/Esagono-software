...
void setup()
{
  ...
  Serial.write("\r\nInput node number ro test its mapping.");
  ...
  while (1)
  {
    ...
    {
      esagono_node_blow(&nodes[i], colors[color_num], 20);
      esagono_node_blow(&nodes[i], CRGB::Black, 20);
    }
    color_num = (color_num + 1) % ARRAY_SIZE(colors);
  }
}
